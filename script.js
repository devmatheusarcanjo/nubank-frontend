...
document.getElementById("esconder-saldo").addEventListener("click", esconderSaldo);
...
function inserirInformacoes() {
    ...
    infoInArray.forEach((ele, index, array) => {
        const valor = ele.value, element = ele.element;
...
function aumentarValor(encrementar, animacao = true) {
    ...
    function acrescentarGradual() {
        ...
        if (total <= info.saldoDebito.value)
            clearInterval(loop)
